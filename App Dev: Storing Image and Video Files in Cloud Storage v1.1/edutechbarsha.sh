...
echo "${CYAN_TEXT}${BOLD_TEXT}                **SUBSCRIBE EDUTECH BARSHA**              ${RESET_FORMAT}"
...
npm start# Function to display colored messages
print_message() {
  echo -e "${BOLD_TEXT}${2}${1}${RESET_FORMAT}"
}
...
# Start the application
print_message "\n🚀 Starting the application..." "${CYAN_TEXT}"
print_message "\nThe application will be started. Complete these manual steps:" "${BOLD_TEXT}${WHITE_TEXT}"
...
print_message "3. Click Create Question link and fill out the form with:" "${RED_TEXT}"
...
print_message "   - Image: Upload the Google-Cloud-Storage-Logo.svg file" "${WHITE_TEXT}"
...
print_message "   - Answer 4: Container Engine" "${CYAN_TEXT}"
print_message "4. Click Save" "${CYAN_TEXT}"
print_message "5. Return to Cloud Console > Navigation menu > Cloud Storage" "${WHITE_TEXT}"
print_message "6. Verify your new object in the bucket named ${DEVSHELL_PROJECT_ID}-media" "${WHITE_TEXT}"
